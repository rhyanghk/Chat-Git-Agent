...
| 什么时候使用 | 第一次使用 Chat-Git-Agent 搭建自己的 workspace 时 |
| 禁止用途 | 不假设用户使用任何特定仓库名（如 `youling/Chat-Git-Agent` / `youling/control project`）；不自动创建仓库；不引入数据库 |

## 首次使用步骤

1. **准备 governance repo** —— 规则、协议、Agent Interface 所在的仓库。
   - 可以直接 fork / clone `Chat-Git-Agent` 作为起点，也可以自建。
