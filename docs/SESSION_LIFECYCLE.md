...
## PROFILE-0001 addendum

- 长期控制角色是 Human、Global Architect 与 Project Architect；执行角色可替换、可丢弃。
- durable knowledge 位于命名 authority source、业务项目或 github_relay 的项目远端；Chat transcript 只是 working memory。
- 协作控制项目与业务项目仓库分离。Agent Skill 不承担控制项目职责。
- 手工传递只传编号化 pointer；接收方从正式来源读取合同。
- 不使用内容哈希、备份副本、镜像或防御性写入。
- github_relay 的 cold bootstrap 和 resume 都先刷新远端并同步完整声明项目树。

---

# Session & Context Lifecycle

Chat-Git-Agent v2.0.0
...
- **Durable knowledge / state**：全局规则（Chat-Git-Agent）、控制平面、项目仓库及其远端 commit / PR / 文档。唯一可恢复事实来源。
...
> 本文档历史使用了 L0→L1→L2 三层称呼，含义等价：本文的 "L0" 即宪法 L0（只指 slim `AGENTS.md`，**不指通读整个 Chat-Git-Agent 或控制面文档**）；本文未单列 L3（rationale/archive），该类内容默认不属于任何 Bootstrap 层。

### L0 — Rules / Identity

启动必读，成本固定且小。L0 只指 **slim `Chat-Git-Agent/AGENTS.md`（机器 L0）+ 精确任务指针**，不要求 Agent 通读整个 Chat-Git-Agent 代码库或完整控制面文档：

- 全局 L0 规则（`Chat-Git-Agent/AGENTS.md`，slim L0，不递归读其他 Chat-Git-Agent 文档）；
...
原则：
- 命名的 durable authority source 是持久事实源；GitHub 仅在 github_relay 时承担项目中继；旧聊天不是任务合同。
...
2. 读全局 L0 规则（Chat-Git-Agent/AGENTS.md，slim L0，不递归读整个 Chat-Git-Agent 或完整控制面文档）。
...
只刷新：
1. fetch 最新全局 L0 规则（Chat-Git-Agent/AGENTS.md）与当前控制平面最小入口；
...
只读取：
1. 全局 L0 规则（Chat-Git-Agent/AGENTS.md，slim L0，不递归读整个 Chat-Git-Agent）；
...
- 访问路径属于寻址元数据：当 pointer 指向 GitHub 且 public / private 会影响启动路由时，seed / dispatch 应显式给最小 access hint（如 `access: github-private | github-public`），不得要求 Agent 先试错探测；该 hint 不承载任务合同，不成为第二 SSOT；机制保持通用，不绑定 control project；
- 原则适用于 Builder / Reviewer / Verifier / Architect / Runner 等，不绑定 control project；control project 只做自身映射；
...
每个项目必须把自身可复现所需的知识放回项目仓库，而不是复制进 Chat-Git-Agent / control project。
...
不强制所有项目使用同一种文件名或工具；优先使用项目原生机制。Chat-Git-Agent 不复制 Python / Node / Docker 等项目专属安装清单。
