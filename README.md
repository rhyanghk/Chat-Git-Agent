...
| Chat 控制模型 | 已配置的 [PROJECT_INSTRUCTIONS.md](00_CHAT_CONTROL/PROJECT_INSTRUCTIONS.md) → [CONTROL_RUNTIME.md](00_CHAT_CONTROL/CONTROL_RUNTIME.md) → 当前正式记录 | 自称 Human、执行代码、验收或发布 |
...
| `00_CHAT_CONTROL/PROJECT_INSTRUCTIONS.md` | Chat 平台 | Human 将正文放入项目指令后，每次控制会话加载 |
| `00_CHAT_CONTROL/CONTROL_RUNTIME.md` | Chat 控制模型 | Human 将其作为项目静态资料上传或提供；项目指令要求每次控制会话先读它 |
| `60_AGENT_SKILL/agent-executor/SKILL.md` | Agent 平台 | Skill 被调用或与编号执行任务匹配时 |
| `60_AGENT_SKILL/agent-executor/references/` | `agent-executor` | Skill 在开始、角色、编号、恢复或传输场景按需读取 |
| `60_AGENT_SKILL/agent-executor/agents/openai.yaml` | 支持元数据的平台 | 展示 Skill 名称、说明和默认调用提示时 |
| `60_AGENT_SKILL/scripts/` | Human / 维护 Agent | 明确执行安装或打包命令时 |
...
| Agent Skill 安装目录 | `agent-executor` 及执行规则 | Chat 派发规则、Human 决策、业务项目全量资料 |
